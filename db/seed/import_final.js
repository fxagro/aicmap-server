...
['Kepulauan Seribu','kotseribu',-5.7167,106.5500,6000],
['Sumba','waingapu',-10.0000,120.2000,10000],
['Flores','maumere',-8.6000,122.2000,10000],
['Timor','kupang2',-10.0000,124.0000,10000],
['Halmahera','ternate',0.7833,127.3833,10000],
['Seram','ambon2',-3.3000,129.3000,8000],
['Buru','namrole',-3.4000,127.0000,6000],
['Tanimbar','saumlaki',-7.9667,131.3833,6000],
['Kepulauan Aru','dobo',-6.1000,134.2167,6000],
...
['Bintuni','bintuni',-2.1500,133.5333,6000],
['Mansel','ratahan',-1.1000,134.3000,6000],
...
['Dogiyai','deiyai',-4.0500,135.6000,6000],
['Yahukimo2','yahukimo',-4.7500,139.2500,6000],
['Pegunungan Bintang2','pbintang',-4.6000,140.4500,6000],
['Mamberamo Tengah','mamberamo',-2.8500,138.6500,6000],
['Yalimo2','yalimo',-3.8500,139.4000,6000],
];
...
async function main(){
  ...
  for(let i=0;i<C.length;i++){
    ...
    let dt;
    try{dt=await fo(q);}catch(e){console.log('['+(i+1)+'/'+C.length+'] '+nm+': ERR-'+e.message.substring(0,100));await new Promise(r=>setTimeout(r,5000));continue;}
    ...
    for(const h of hl){
      try{
        ...
        console.log('  INSERT ERR['+h.name+']:',e.message.substring(0,120));
      }
    }
